...
outDir='~/opentrons_testing_run/05_dedup_counts/'
...
for(name in list.files(alignDir)[grep('001.align.bamAligned.sortedByCoord.out.bam.featureCounts.sam', list.files(alignDir))]){
	...
	aligN=aligN[!duplicated(paste0(aligN$V1,aligN$V3, aligN$V4, aligN$V18)),] # deduplicate with or without UMIs here
...
exprDir='~/opentrons_testing_run/05_dedup_counts/'
inL=list()
i=1
...
countBrake=t(data.frame(rRNA=apply(out[grep('^rRNA_*', out$type), -seq(1,3)], 2, sum), mtRNA=apply(out[grep('^Mt_*', out$type), -seq(1,3)], 2, sum), other_RNA=apply(out[grep('^scaRNA|^scRNA|^snoRNA|^sRNA|^miRNA|^lncRNA|^misc_RNA', out$type), -seq(1,3)], 2, sum), protein=apply(out[grep('protein_coding', out$type), -seq(1,3)], 2, sum), all=apply(out[, -seq(1,3)], 2, sum)))
...
plot=melt(countBrake[c(1,2,3,5,4),])
pdf('features.pdf')
