...
sample=list(filter(lambda x:'R1' in x, sample))
sample=[re.sub("_R1.*", "", x) for x in sample]
...
rule umi_tools:
    ...
    output:"01_basecalled/{file_names}.fastq.gz"
    ...
    shell:
        ...
        --bc-pattern=NNNNNN \\
        -L {log} \\
        --stdin=01_basecalled/{wildcards.file_names}_R1_001.fastq.gz \\
        --stdout {output}
        """
...
rule genebody_cov:
    ...
    shell:
        ...
        -o 09_sort_index/{wildcards.file_names} \\
        -f pdf
        """
